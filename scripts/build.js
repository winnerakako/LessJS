...
const envFiles = ['env/.env', 'env/.env.dev', 'env/.env.staging'];
...
envFiles.forEach((envFile) => {
  if (fs.existsSync(envFile)) {
    try {
      const destFile = envFile.replace('env/', ''); // Remove env/ prefix for dist
      fs.copySync(envFile, `dist/${destFile}`);
      console.log(`✅ Copied ${envFile} to dist/${destFile}`);
    } catch (error) {
